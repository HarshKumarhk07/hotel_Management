...
/**
 * Role-neutral orders table.
 *  - `showKitchenFilter` (Admin) exposes a cross-kitchen picker.
 *  - Without it (Kitchen portal) the list omits any kitchen param, so the
 *    backend force-scopes results to the caller's own kitchen — a spoofed or
 *    omitted kitchen id can never widen the result set.
 */
export function OrdersList({ showKitchenFilter = false }: { showKitchenFilter?: boolean }) {
  const [status, setStatus] = useState('');
  const [kitchen, setKitchen] = useState('');
  const [openId, setOpenId] = useState<string | null>(null);
  const { data: orders, isLoading } = useAdminOrders({ status, kitchen: showKitchenFilter ? kitchen : '' });
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          {showKitchenFilter && <KitchenSelect value={kitchen} onChange={setKitchen} allowAll />}
          <select
